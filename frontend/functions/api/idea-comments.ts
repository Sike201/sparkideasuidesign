// File: functions/api/idea-comments.ts
// Comments API for ideas
...
// GET - Fetch comments for an idea
async function handleGetRequest(ctx: EventContext<ENV, string, unknown>) {
  ...
  try {
    ...
    if (!ideaId) {
      return jsonResponse({ message: "Idea ID is required" }, 400);
    }

    // Fetch all comments including replies with vote counts and author investment, sorted by creation date
    // Note: We'll calculate author investment separately since we need to match by wallet address
    ...
    // First try by twitter username (wallet-independent), then fallback to wallet lookup
    const commentsWithInvestment = await Promise.all(
      (comments.results || []).map(async (comment: any) => {
        let authorInvestment = 0;

        // Try by twitter username first (works even if wallet changes)
        ...
        // Fallback: lookup via wallet if no username match (for older investments)
...
async function handlePostRequest(ctx: EventContext<ENV, string, unknown>) {
  ...
  try {
    ...
    const { ideaId, parentCommentId, content, authorUsername, authorAvatar, authorTwitterId, isTeam, walletAddress } = body;

    // Validate required fields
    if (!ideaId || !content) {
      return new Response(
        JSON.stringify({ message: "Idea ID and content are required" }),
        {
          status: 400,
          headers: {
    ...
    if (parentCommentId) {
      const parentComment = await db
        .prepare("SELECT id FROM idea_comments WHERE id = ?")
    ...
    await db
      .prepare(
        `INSERT INTO idea_comments (id, idea_id, parent_comment_id, content, author_username, author_avatar, author_twitter_id, is_team, created_at)
  ...
  try {
    ...
    if ((action === "vote" || action === "upvote" || action === "downvote") && userId) {
      ...
      const existingVote = await db
        .prepare("SELECT * FROM idea_comment_votes WHERE comment_id = ? AND user_id = ?")
        .bind(id, userId)
      ...
      if (existingVote) {
        ...
        if (existingVote.vote_type === actualVoteType) {
          await db
            .prepare("DELETE FROM idea_comment_votes WHERE comment_id = ? AND user_id = ?")
            .bind(id, userId)
            .run();
          return new Response(
          ...
          await db
            .prepare("UPDATE idea_comment_votes SET vote_type = ?, voter_twitter_id = ?, voter_username = ? WHERE comment_id = ? AND user_id = ?")
        ...
        await db
          .prepare(
            "INSERT INTO idea_comment_votes (id, comment_id, user_id, voter_twitter_id, voter_username, vote_type, created_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)"
          )
...
async function handleDeleteRequest(ctx: EventContext<ENV, string, unknown>) {
  ...
  try {
    ...
    await db.prepare("DELETE FROM idea_comments WHERE id = ?").bind(id).run();
